...
const fixtureUnionpeerHTML = `<html><head><title>Сборник :: Unionpeer.org</title></head>
<body>
...
// the mirror recorded in the stored topic URL — unionpeer has no id-based
...
func TestCheck_RewritesToActiveDomain(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(200)
		_, _ = w.Write([]byte(fixtureUnionpeerHTML))
	}))
	t.Cleanup(srv.Close)
	...
	topic := &domain.Topic{URL: "https://unionpeer.org/forum/viewtopic.php?t=12345"}
	...
	if rec.hosts[0] != "unionpeer.net" {
		t.Errorf("fetch host = %q, want active domain unionpeer.net", rec.hosts[0])
	}
}

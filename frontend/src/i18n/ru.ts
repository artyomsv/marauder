const ru: Record<string, string> = {
  ...
  "topics.reset.body":
    "Удаляет записи о загрузках, прогресс по сериям и состояние ошибки, затем сразу проверяет тему заново, чтобы текущий выпуск считался новым. Темы с загрузкой скачают его снова; темы «только уведомления» просто пришлют уведомление. Настройки и история событий сохраняются. Приостановленная тема останется приостановленной.",

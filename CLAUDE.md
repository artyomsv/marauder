...
`WithRawPage` (Tapochek only so far; issue #186) returns the topic page exactly as that plugin's `Check` reads it — same session, same active domain, same charset decode — so a user can export it for a bug report via `POST /api/v1/topics/{id}/diagnostics/page` (owner-only, per-topic single-flight, redacted through `pageredact`). Implementing it is a ~5-line method that routes through the plugin's own canonicalise-and-fetch pair; a convenient plain GET is NOT acceptable, because a page collected without the session or the decode looks like evidence and is not. `supports_raw_page` is surfaced per tracker in `/system/info` so the frontend hides the action rather than offering a control that can only 409.

...
A reset that lands **mid-check** is protected by optimistic concurrency on
`last_checked_at` — written by exactly two statements, `RecordCheckResult` and
`ResetCheckState`, which makes it a version token. `RecordCheckResult` carries
the value its worker observed at dispatch and guards on
`last_checked_at IS NOT DISTINCT FROM $7` (`IS NOT DISTINCT FROM`, not `=`, so
the post-reset `NULL` still matches and the fresh check can write); a
non-matching write affects zero rows and returns `repo.ErrStaleCheckResult`,
which the scheduler logs at Info and drops. Without it the finishing check
would restore the pre-reset hash and a backoff `next_check_at`, leaving a topic
whose torrents are already gone from the client but which never re-downloads.

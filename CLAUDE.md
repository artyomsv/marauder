...
deploy/         docker-compose stacks (source-build base + prebuilt-image
                ghcr stack + dev + sso + solver overlays + test-clients
                matrix + arr stack for the Sonarr integration) + Kubernetes:
...
| `cfsolver` | in-process client to the standalone `cfsolver/` service. **Unused** — superseded by the `flaresolverr` minter, which does the same solve-then-hand-cookies-to-Go job against a maintained solver. Note that model is now known to *work*: it was written off on 2026-07-28 as impossible (`cf_clearance` believed TLS-fingerprint-bound), but the binding is to the User-Agent and replay from Go succeeds when the UA is repeated (measured 2026-08-01) |
...
| **`flaresolverr`** | client for a FlareSolverr instance, used as a **Cloudflare clearance minter**: `Clearance(ctx, probeURL)` solves the managed challenge once and returns `cf_clearance` + the browser User-Agent, cached per host and installed process-wide via `registry.SetClearanceProvider`. Plugins replay both on ordinary Go requests — the cookie is bound to the **User-Agent** (and the egress IP), NOT to the TLS fingerprint, so replay from `net/http` works on every gated path including the binary `dl.php` (measured 2026-08-01; the earlier "TLS-bound" finding was a UA mismatch — the test replayed the cookie while sending the plugin's hardcoded `Marauder/0.3`). A clearance is scoped to the Cloudflare **rule** that issued it, not the host: minting from RuTracker's root (which 301s to the unchallenged `/forum/index.php`) yields a cookie that still 403s on `/forum/login.php`, so `probeURL` must be a genuinely challenged page. A stale clearance surfaces as a 403 `Cf-Mitigated`, which `rutracker.fetchBytes` answers by invalidating and re-minting **exactly once**. Holds one long-lived solver session (`sessions.create`, released by `Close`) because a session-less solve re-runs the challenge every call (10-20s) and FlareSolverr serialises. `minSolveBudget` (20s) is the floor below which `solveBudget` refuses with `ErrBudgetTooShort` rather than starting a solve the caller will abandon — set from measurement (live RuTracker challenges took 10.9-13.4s on 2026-08-05), so it sits **above** the tracker-search handler's 15s per-tracker budget: a cold search cannot mint and depends on the boot warm-up, which is deliberate — it previously timed out at 15s anyway, and the abandoned browser then blocked the next caller. `Transport.RoundTrip` (fetch *through* the browser) is retained but unused: GET-only and string-typed, so it cannot carry a `.torrent`. **Boot ordering:** the backend's warm-up runs ~20ms after start while FlareSolverr needs ~9s to launch Chrome, so the solver/dev/arr Compose stacks give it a `/health` healthcheck and the backend a `depends_on: {condition: service_healthy, required: false}`. That orders Compose only — k8s and the standalone arr project cannot be ordered — which is why the durable fix is `registry.ErrClearanceUnavailable` making the failure self-correct in 60s |
| **`plugins/cfclearance`** | the Cloudflare-clearance dance shared by tracker plugins whose sites answer a plain Go client with a managed challenge. Extracted from `rutracker` on 2026-09-03 when **kinozal** started being challenged too (`/browse.php`, `/details.php`, `/get_srv_details.php` all answer 403 `Cf-Mitigated`, while the site root still returns 200) — a second copy was the wrong kind of duplication because every branch of it was paid for by a real incident. `Apply(ctx, plugin, jar, u, probeURL)` seeds the jar and returns the clearance so the caller can send the matching User-Agent, and returns the **provider's** error rather than logging and dropping it — that error is the only thing that later lets `Cause` blame the solver. Cookies are marked `Secure` only on an https origin, so the jar cannot replay a clearance onto a plaintext hop while the plugins' httptest servers still work. `UserAgent(c, fallback)` picks the clearance's UA over the plugin's own. `IsChallenge(resp)` reads `Cf-Mitigated` + 403/503 rather than sniffing body text. `Cause(clearErr)` is the three-way split (`ErrClearanceUnavailable` / `ErrClearanceNotConfigured` / `ErrCloudflareChallenge`) that issue #158 paid for. `RetryOnChallenge(probeURL, attempt)` invalidates the cached clearance and retries **once** — and only on `ErrCloudflareChallenge`, since the other two do not wrap it and there is nothing to re-mint from |
...
| `domain` | core types: `Topic` (incl. per-topic `ClientID`, **`NotifierID`** (per-topic notifier override, migration `0007`), `DownloadDir`, `Category`, `ImageURL`, **`ReplaceOnUpdate`/`ReplaceDeleteData`** (replace-on-update policy, migration `0014`), **`NotifyOnly`/`NotifyOnlyAnnounceCurrent`** (notify-only watch mode, migration `0016`)), `Check`, `Payload`, `TrackerCredential`, `TopicDelivery`, `AddOptions` (`DownloadDir` + `Category`) |
...
| **`pageredact`** | strips secrets out of tracker HTML before Marauder hands it to a user to attach to a bug report (`POST /topics/{id}/diagnostics/page`). Since the export was narrowed to the regions a parser reads (see `WithPageExport` below) it is a **backstop**: on live tapochek pages those regions carry no secrets at all, so it normally changes nothing — keep it anyway, because a template can move a token into a region the parser reads. Born from issue #186, where five days went by because the only copy of the page showing the bug belonged to the reporter. Its contract has **two** halves and the second is the one that gets broken: no known secret survives, **and every other byte is untouched** — tags, classes, attribute order, quotes, whitespace and entity spellings are what a report is FOR. **It is not regex-based, and must not go back to being so.** It was, for three Greptile rounds on PR #193, and two of the three P1s were regressions introduced by the previous round's fix (name-before-value only → quote-aware scan let an unterminated quote swallow the next tag → bounding quoted runs at `<` broke `title="1 < 2"`); an independent codex review then reproduced 11 more (5 P1). Now `golang.org/x/net/html`'s tokenizer is used **only for token boundaries and kinds** — the page is never parsed into a tree and never re-serialised, because rendering lowercases tags, re-quotes attributes and re-encodes entities. Content is read from the ORIGINAL page by offset (`len(z.Raw())` is the only thing taken from each token, so the tokenizer's buffer ownership never matters), attributes are read by a small HTML-grammar lexer that records exact byte offsets (`tag.go` — the tokenizer reports no offsets and silently drops duplicate attributes a public file still carries), classification runs on **decoded** text whose every byte maps back to its source span (`text.go`, so `form_to&#107;en` is `form_token` and the replacement still lands on `&#107;`), and all decisions become span edits applied once to the original (`edits.go`), which is what makes a second pass a no-op. What it removes: credential-named fields' `value`/`content` on real form elements only (`<input>`, `<meta>`… — not `<input-widget>`, not example markup in a comment/textarea/CDATA), attributes whose own name is a credential (`data-sid`), `key=value` query params, JS/JSON `key: "value"` pairs, echoed `Cookie:`/`Authorization:` lines (joined across inline markup, stopped at the line or attribute end), and the reporter's tracker username — in text and non-structural attributes only, never `class`/`id` (a username of `seedmed` must not erase the class that solved #186), with Unicode-aware edges. Credential names are matched as **words** (split on non-alphanumerics and camelCase), so `authToken`/`session_id` count and `author`/`keywords`/`monkey`/`consideration` do not. Ambiguity is resolved by redacting, not refusing: a tag with a duplicate attribute, a quote in an attribute name, or no closing `>` gets a second raw-text pass plus every `<letter` inside lexed as its own tag, and a duplicated `name` counts as credential if ANY copy is (a deliberate divergence from browser first-wins semantics, pinned by a test). `Redact` returns `ErrUnsafe` only if the tokens do not cover the page; the handler maps it to 422 and returns nothing rather than unexamined bytes. Verified on three real tapochek pages (106-190 KB, up to 8.7k tokens): 4 secrets each (`form_token`, two hidden `name="sid"` fields, the `sid=` link), **zero** structural differences. Tests: `adversarial_test.go` holds the review's exact inputs and outputs; all but one fail against the regex version, and that one pins the duplicate-name policy. **Every loop must stay linear** — it runs synchronously inside a request, and the second review measured two that were not: re-lexing a malformed tag to its end once per swallowed `<x` (48 KB took 42 s; each swallowed tag is now lexed only up to the next one) and absorbing placeholders by comparing every placeholder with every edit (a 3.8 MB second pass took 17.5 s; now one forward sweep over sorted, merged edits). `scaling_test.go` pins both with bounds far above linear and far below quadratic |
| **`trackercreds`** | `Warm(ctx, store, master, userID, tracker)` — load, decrypt and warm a user's stored credential so a caller can read a tracker page AS that user. Its own package because four callers in three packages need it (tracker search, the AddTopic preview, `POST /topics`, the Sonarr poller) and the poller must not import `api/handlers`. It exists because a login-gated tracker does **not** answer an anonymous caller with an error — it answers with a stub (Toloka: a ~7KB page with an empty `<title>`), so an anonymous resolve does not fail, it succeeds at reading nothing and stores a topic with a placeholder name and no image, which nothing backfills. Every failure degrades to nil creds; no caller may hard-fail on credential trouble. The session step is `EnsureSession(ctx, tracker, creds)`, shared with the scheduler's `loadCredentials`: **Verify-first, Login-on-miss** (a warm session pays one cheap GET; only a cold one pays the login), with an errored Verify (incl. `ErrVerifyUnsupported`) still falling through to Login, and a process-wide per-`(tracker, user)` lock (a one-slot channel, so a waiter gives up on its own ctx) so at most one Verify/Login for that session is in flight. Issue #198: the scheduler used to `Login` before every check, and a burst of one user's due Tapochek topics sent four logins inside 20ms that the site answered 503. Verify-first alone does not stop that burst — after a restart or session expiry every worker sees the same cold session at once — so the first caller logs in and the rest queue. **A queued caller reuses the answer of an attempt for the same credential that finished after it arrived** — success or refusal — so N topics due together cost one round-trip, not N in a row on budgets already running (PR #200 review round 2: serialised Verifies timed healthy checks out, and a refused login was asked N times). An attempt ended by its own caller's context is never handed on. **Deliberately a lock, not `singleflight`** (the first version, rejected in review round 1): callers' budgets range from 5s (preview) to 35s (a scheduler check), and a shared attempt failed the long caller on the short one's deadline; here each caller waits and works on its own context. **Verify is trusted only for the credential that established the session**: the slot remembers a SHA-256 fingerprint of username + password + session blob from the last successful Login, and any mismatch — a credential edit, or a cold slot after a restart — goes straight to Login. That is load-bearing, not an optimisation: plugins key their session by Marauder user, not tracker account, and `Verify` asks "signed in?", not "as whom?", so after a username edit the old account's session still verifies. A failed Login clears the owner. The credentials handler's `loginAndVerify` (Login→Verify always, right for validating a freshly entered password) stays separate but runs inside **`Establish(ctx, tracker, creds, login)`** — same lock, and records the owner on success — so a session the create/update/test endpoints sign in counts for the scheduler; without it the scheduler would log in again at once, and fail if the password had since changed on the tracker. Any new code path that calls a plugin's `Login` directly must go through `Establish` too. The second return separates "no credential stored" from "stored but would not warm", which only search surfaces |
| **`topics`** | shared `BuildAndCreate(store, CreateInput)` — the one tracker-match → `Parse` → fail-open `ResolveMetadata` → build → persist sequence, used by BOTH the `POST /topics` handler and the Sonarr poller (no duplication). `CreateInput.Extra` lets trusted headless creators merge tracker metadata over `Parse` defaults before shared capability/source fields are applied. `CreateInput.Credentials` (a func, so the credential is only loaded and its session only warmed when a tracker actually implements `WithMetadata`) supplies the creator's warmed tracker credential to the metadata resolve — nil resolves anonymously, correct for a public tracker and useless for a gated one, which is how a Toloka topic came to be stored with a placeholder name and no poster (the scheduler self-heals a placeholder name on the first check, but **nothing backfills an image**). The warm gets its own `credentialWarmTimeout` budget rather than a slice of `metadataTimeout`, so a slow login cannot spend the fetch's deadline and leave it to fail instantly on a dead context. `SafeImageURL` is the central http(s)-only scheme guard on a scraped poster before it is stored or previewed — the value is persisted once and then rendered into an `<img src>` for every later viewer. Idempotent: a `(user_id,url)` unique-violation returns `Result{Created:false}` instead of erroring. Sentinels `ErrNoTracker`/`ErrParse`/`ErrQualityUnsupported`. `CanonicalTopicURL(tr, rawURL)` (issue #126) rewrites a new topic's host to the tracker's canonical `WithDomains` domain at create time, so adding the same release via a different mirror still dedups against an existing topic; used by both the `POST /topics` handler and the Sonarr poller. **`TopicEvents`** sibling repo (history feed: `Record`/`ListForTopic`/`ListForUserSince`) |
| **`sonarr`** | Sonarr integration (issues #86, #92): a typed read-only API `Client` (`SystemStatus` for the Test button; `GrabHistorySince` reads `eventType=grabbed` history, extracting `data.nzbInfoUrl` — the tracker topic URL, **not** `guid` — plus `sourceTitle`/`data.torrentInfoHash` for variant identity) + a `Poller` that mirrors the scheduler (ticker loop, ctx-cancel, fail-open). Multi-instance: a single manager loop ticks at the 60s floor, lists **enabled** `sonarr_instances`, and polls any instance due per its own interval (per-id `lastPolled` map, pruned when an instance is removed) — add/edit/enable/disable/delete take effect on the next tick. Per instance it resolves the owner (configured admin ⇒ first admin), dedups history by URL while retaining the newest grab's variant metadata, refreshes that metadata on later grabs of an existing Sonarr-owned topic independently of client/category/path realignment, filters by allowed trackers, and auto-creates topics via `topics.BuildAndCreate` with that instance's default client/category/dir, advancing **its** cursor. Metadata is resolved as the topic's **owner** via `trackercreds.Warm` (fail-open to anonymous), because a gated tracker answers a guest with a stub rather than an error — resolving as nobody stored a placeholder name and no poster, and nothing backfills a poster. First enable per instance is go-forward only (cursor stamped, no historical import). Admin config UI: **Integrations** page (`pages/Integrations.tsx` → `components/integrations/SonarrInstanceList` → `SonarrInstanceCard`/`SonarrInstanceForm`); collection API `GET/POST /api/v1/system/sonarr/instances`, `GET/PUT/DELETE /api/v1/system/sonarr/instances/{id}`, `POST .../{id}/test`, ad-hoc `POST /api/v1/system/sonarr/test` |
...
| `plugins/registry` | plugin interfaces + global registry + tracker capability interfaces (`WithQuality`, `WithEpisodeFilter`, `WithCheckSpacing` — minimum gap between a tracker's check starts, issue #198; `WithCredentials`, `WithCloudflare`, `WithInteractiveLogin`, `WithSeasonCatalog`, `WithMetadata`, `WithAuthorComment` — release author's latest thread comment for update notifications, issue #110; RuTracker + NNM-Club, parsing helpers in `forumcommon/posts.go`; `WithDomains` — `Domains() []string`, canonical domain first then known mirrors, issue #126; implemented by the 9 fixed-domain plugins) + client capabilities `WithStatus` (`Status(ctx, rawConfig, hashes) []TorrentStatus` — live download status by infohash; normalised `State*` vocabulary) and `WithCategories` (`Categories(ctx, rawConfig) []string` — list the categories a client already knows about, for the AddTopic category combobox; qBittorrent-only today, served by `GET /api/v1/clients/{id}/categories`, fail-open) and `WithRemoval` (`Remove(ctx, rawConfig, hashes, deleteData)` — remove torrents by infohash, optionally deleting on-disk data; powers the replace-on-update policy (issue #101); implemented by all networked clients) + the `registry.EffectiveDownloadDir(base, override, category)` save-path helper + the process-wide `SetDomainResolver`/`ActiveDomain`/`DomainAllowed` hook (wired to `domains.Store`; issue #126) that a `WithDomains` plugin uses to resolve its admin-configured active domain and validate scraped/redirect hosts against its known-plus-custom allowlist + typed sentinels **`registry.ErrNoPendingEpisodes`**, `ErrCaptchaRequired`, `ErrSessionExpired`, `ErrCloudflareChallenge` (response was a Cloudflare interstitial, not the page — returned by RuTracker's `Login`/fetch so a network block is never reported as bad credentials), **`ErrClearanceUnavailable`** (the request was challenged **and** the configured clearance provider had already failed to mint — blames the solver, not the tracker. `rutracker.challengeCause(clearErr)` picks between the two: `applyClearance` now returns the provider's error instead of logging and dropping it, and a non-nil one means this request was always going to be blocked. Deliberately does **not** wrap `ErrCloudflareChallenge`, so `retryOnChallenge` skips its invalidate-and-refetch — there is no cached clearance to drop and a second doomed request only loads an already-unwell solver. The scheduler treats it as transient (`isTransientError`), so the topic retries in 60s instead of taking the exponential path), **`ErrClearanceNotConfigured`** (the request was challenged and **no** provider is installed at all — the operator never set `MARAUDER_FLARESOLVERR_URL`. Distinguished from `ErrClearanceUnavailable` via the new `registry.ClearanceConfigured()` query, because `ClearanceFor` deliberately reports "no provider" as `(zero, nil)` so ungated paths keep working, which leaves the challenged caller unable to tell an unconfigured deployment from a solver that answered. Issue #158: every shipped stack wired the container and the env var separately and none wired both, so this state was universal and reported as `cloudflare` — "this tracker needs a browser to get through", about a browser nothing had been asked to run. Classified `solver_missing`, and unlike `ErrClearanceUnavailable` it is **not** transient: a missing setting does not fix itself in 60s, so it takes the ordinary exponential backoff. Like it, it does not wrap `ErrCloudflareChallenge`, so `retryOnChallenge` is skipped — there is no provider to re-mint from), **`ErrVerifyUnsupported`** (returned by a `WithCredentials` plugin whose `Verify` cannot determine session state — AniDub, for a page it cannot classify. Toloka returned it until 2026-09-03, when its `toloka_data` cookie turned out to carry a `userid` that answers the question outright; prefer hunting for a server-supplied marker like that before reaching for this sentinel. It exists because the alternative such plugins reach for is `return true, nil`, which is indistinguishable from a real positive and collapses the credentials handler's deliberate two-signal design down to `Login`'s negative check, which fails open on any wording it does not recognise. `loginAndVerify` maps it to `(verified=false, err=nil)`: the credential is saved and usable, but the API reports `verified:false` and the UI shows an amber "could not be verified" notice instead of a green tick. **Never add a `Verify` that returns `true` without making a request** — return this sentinel instead) |
...
| `plugins/trackers/<name>` | one package per tracker plugin (13 plugins). **Removed 2026-08-03** after probing every advertised domain: `hdclub` (site shut down 2017; `hdclub.org` now 301s to an unrelated file-hosting affiliate site that 404s every TBDev endpoint), `unionpeer` (`.org`/`.com` are parking pages, `.net` NXDOMAIN) and `freetorrents` (`free-torrents.org` has no A record). Do not re-add without live evidence the site serves a topic page. **`toloka` rebuilt 2026-09-03**: it had never worked. Release pages publish no infohash and no magnet (four live releases checked), so its `Info hash: <40 hex>` regex — an English label absent from this Ukrainian site — meant every check failed with "no infohash found"; `Check` now derives an anidub-style change token from the `class="btTbl"` block (download id + `.torrent` filename + size + registration timestamp, seeder counts excluded so they cannot fake an update, and both the size and date regexes step from the label cell straight into the value cell because a lazy `.*?` wanders into `<span title="Розмір частини: 2&nbsp;MB">` and tracks the piece size). `Login`/`Verify` read the server's `toloka_data` cookie (`userid` −1 for a guest, the account id when signed in) instead of grepping the body: a successful login answers **302 with an empty body** and the real failure page contains neither "помилка" nor "error", so the old check reported a rejected password as success. Everything is login-gated — a guest gets a ~7KB stub with an empty `<title>`, `tracker.php` returns zero rows with no error, and `download.php` serves HTML — so a missing torrent block asks `Verify` and reports `registry.ErrSessionExpired` rather than a parse failure, `Download` rejects a non-bencoded body, and `Search` refuses without credentials instead of reporting "no results". `toloka.to` is the only live domain (hurtom.com is the sister portal, not a mirror), and the tracker rate-limits hard — 6 requests in 3s earns an HTTP 429 (and so does ~12 over 20s), which `do` names explicitly, keeping the `-> 429` marker so `classifyError` still reads the status. **`rutor` re-domained 2026-09-03**: `rutor.org` is a stale clone (newest id 1087871 vs 1104882 on the live mirrors, ~17k releases behind, `404 Раздача не существует` on every current id), so `defaultDomain`/`knownDomains` moved to `rutor.info` + `new-rutor.org` and `rutor.org` survives only in `parseDomains` — keeping it out of `knownDomains` is what stops domain rotation landing on a mirror it can never leave. The two live mirrors share one id space, but only `rutor.info` serves `.torrent` bytes — on both `d.rutor.info/download/<id>` and plain `rutor.info/download/<id>` — while `new-rutor.org` serves neither (`d.new-rutor.org` has a mismatched certificate; its own `/download/<id>` answers 200 with HTML), so `torrentURLs` walks `d.<active>`, `<active>`, `d.<canonical>`, `<canonical>` (deduped, canonical taken from `defaultDomain` so `Domains()` ordering cannot silently redirect the fallback). Each candidate gets its **own** `torrentAttemptTimeout` (8s) slice of the caller's context rather than sharing it: the scheduler hands `Download` a single `TrackerHTTPTimeout` for the page fetch and every candidate together, so a host that *stalls* rather than refusing — a blackholed mirror, which is what a dying one does — would otherwise consume the whole budget and leave the reachable canonical host failing instantly on an expired context, taking the magnet fallback while a working `.torrent` was one request away (measured: 10s budget, one candidate tried). The loop also breaks on an already-dead parent so the exhaustion Warn does not blame the mirrors for our own expired deadline. `Download` accepts a fetched `.torrent` **only** when its infohash matches the page magnet — Check derives the topic hash from that magnet, so a mismatched file would record a delivery that never matches the check — and otherwise fails open to the magnet; exhausting every host logs a **Warn**, since a permanently broken download host would otherwise downgrade every rutor delivery silently at Debug level. It returns the file **without** a magnet, per `domain.Payload`'s "exactly one of" contract and because every client plugin resolves `MagnetURI` first when both are set. The magnet is matched in its **HTML-escaped** page form (`&amp;`) and unescaped: stopping at the bare `&` stripped every `&tr=` announce URL, leaving a DHT-only magnet on precisely the fallback path that runs when the download host is broken. Its btih is parsed by `infohash.FromMagnet` rather than a second regex, so Check and Download provably agree and a base32 or malformed hash fails loudly instead of being truncated into a plausible-looking one. Magnet and poster are read from `<div id="download">` and `<table id="details">` via `forumcommon.TagBlockInner` (fail-open to the whole page) because the "similar releases" list carries magnets for **other** releases; `cleanTitle` strips a `<host> :: ` prefix only when the host is in `parseDomains`, since dotted release names (`Halo.3::ODST`) are hostname-shaped; `canonicalURL` forces `https`; and the client's `CheckRedirect` re-runs `fetchHostAllowed` on every hop, so the host guard covers redirects and not just the initial URL **`tapochek` rebuilt 2026-09-04**: the last fixture-only plugin, and it had never worked either — same `Info hash: <40 hex>` regex against a site that publishes no infohash and no magnet to a guest OR a member (five live release pages checked), so `Check` now derives the same anidub/toloka change token from the `<table class="attach">` block (download id + `.torrent` filename + release size + registration timestamp; download and thanks counts excluded). `Login`/`Verify` read the server's `bb_data` cookie, whose `uid` field is the account id and which a guest is issued **not at all**, replacing a `logout.php?sid=` marker this site never emits (so `Verify` reported every live session as dead) paired with a `Login` that checked neither status nor body (so a rejected password read as success) — both signals broken at once, in opposite directions. Everything is login-gated: most forums 302 a guest to `login.php`, and even a readable topic has its download box replaced with a link to the registration page, so a missing torrent block reports `registry.ErrSessionExpired` and `Download` rejects a non-bencoded body (`download.php` answers an unaccepted session with **200 and HTML**). The site is **windows-1251**, and unlike rutor that is not cosmetic — every label the parser anchors on is Cyrillic, so `fetchPage` transcodes while `fetch` deliberately does not, since decoding the `.torrent` bytes would corrupt the file; the login failure page needs the same decode or a rejected password degrades to a vague "no session established". The site serves **two page templates** and they mark the cover differently, which issue #186 paid for: film/game topics use `<var class="postImg postImgAligned img-right">` (the poster is the FIRST such image in the opening post — a plain `postImg` is a banner, screenshot or rank badge), while TV-series topics carry no aligned `<var>` at all and put the artwork in `<img class="poster">`, their only `<var>`s being Kinopoisk/IMDb rating badges and language flags. `posterURL` tries the `<var>` form first (so a page carrying both keeps the image already stored for it) and falls back to the `<img>` one, which is class-gated because the screenshots in the same post are `<img>` tags too, and which keeps a *relative* `src` for `absoluteURL` to resolve against the active domain (dropping it would leave the topic with no image, the exact failure being fixed) while refusing a scheme that could never name an image — `javascript:`, `data:` — before resolution, since resolving those first would hand the central `topics.SafeImageURL` guard a valid-looking https URL carrying the payload in its path. The **class on the filename `<th>` is per-viewer and must never anchor a selector**: the site colours that header and the download link beside it by the reader's relation to the release — `genmed` for a stranger, `seedmed` for someone who already seeds it, `leechmed` elsewhere on the same page — so `fileNameRe` keyed on `genmed` worked for every account that did not have the torrent and failed for every account that did. That is the real answer to issue #186 and it cost five days: the reporter's three topics checked cleanly from our account throughout, because our account does not seed them. `fileName(block)` now walks the block's `<th>` cells and takes the first whose **normalised** text ends `.torrent` — the suffix identifies the attachment cell, and the release-type banner beside it is rejected for naming no attachment rather than for wrapping its text in `<img>` tags, which is true today but is not the guard. The suffix is deliberately tested AFTER `normalizeCell` rather than inside the pattern: a regex ending `\s*</th>` cannot match a trailing `&nbsp;` (Go's `\s` is ASCII-only) and this table emits those freely, so the first fix would have reintroduced the same total failure the day the filename cell gained one. `fingerprintParts` reporting WHICH field was lost is what made it findable at all — the reporter's `(missing: name)` narrowed a whole page to one cell — so keep that: the generic message must name fields, not just fail. `blockFieldsError` (used by both `Check` and `Download`) additionally words the id-only case as "no download link … this account may not be allowed to download this release", which remains a real state (Tapochek limits downloading by ratio, rank and a daily cap) though it was NOT what #186 turned out to be; its wording is deliberately kept clear of the scheduler's auth and parse keyword sets so it classifies `unknown` and the UI shows the raw detail. `checkTarget` trusts `effectiveDomain()` alongside `registry.DomainAllowed`, which consults the known and custom lists but **not** the admin's active setting — without that the plugin can build every URL against its configured host and then refuse to dial it. |
...
1. `loadCredentials` — fetches and decrypts tracker credentials (if the
   plugin implements `WithCredentials`), then `trackercreds.EnsureSession`
   (Verify-first, Login-on-miss, serialised per tracker+user — issue #198).
...
**Notify-only topics (issue #184):** the `updated` branch in `runCheck` splits:
topics with `NotifyOnly` take `notifyOnlyRelease`, never resolve a download
client, and fall through to the shared tail. `recordResult` persists the **new**
hash because there is no download to retry. The download flow and
`release.found` repeat-emission rule above describe the download path;
notify-only deliberately does **not** gate that emit on `ConsecutiveErrors`,
because suppressing an event here loses it rather than replaying it next tick.
For per-episode trackers, pending episodes are marked seen through
`Topics.MarkEpisodeDownloaded`, so switching back to download mode does not
fetch the backlog. Episodic announcements require a non-empty
`pending_episodes` list: LostFilm's count-derived `eps:N/done:D/pending:P` hash
changes again on the tick after marking, when there is nothing new to announce.
`NotifyOnlyAnnounceCurrent` controls announcement on the first check and again
after `Topics.ResetCheckState` clears `last_hash`. Episode marking is fail-open:
a mark error stops the loop without failing the check; a stale check-state token
also prevents the shared tail from persisting stale results. User guide:
`docs/notify-only.md`.
...
**Domain rotation (issue #126):** `recordResult` classifies a check failure
before persisting it, and a `timeout`/`unreachable` classification calls
`domains.Store.ReportFailure(ctx, trackerName)` to give the tracker's active
domain a chance to rotate to a configured mirror (cooldown-gated on the
store's side, so a burst of failing topics can't spin the ring past a
working mirror).
...
│   │                           admin-only Tracker domains card
│   │                           (components/settings/TrackerDomainsCard)
...
`deploy/docker-compose.solver.yml` is the **Cloudflare solver overlay**: it
starts FlareSolverr *and* sets `MARAUDER_FLARESOLVERR_URL` on the backend, and
layers onto either production stack (`docker-compose.yml` or
`docker-compose.ghcr.yml`). The two halves are inseparable on purpose — issue
#158 was every shipped stack wiring exactly one of them (ghcr: neither service
nor env passthrough; base: env but no service; dev overlay: service, healthcheck
gate, and no env; Helm `arr.flaresolverr`: container but no env), which is
indistinguishable at runtime from having no solver at all. Do **not** layer it
onto the dev overlay, which already runs its own `flaresolverr` service. It is
deliberately not a Compose **profile** on the base file: profiles merge across
`-f` layers rather than being overridable, so a profile-gated base service makes
the dev overlay's unconditional one profile-gated too and breaks that stack.
...
`WithSeasonCatalog`, `WithMetadata`, `WithAuthorComment`, `WithSearch`, `WithCheckSpacing` (Tapochek — see "Tracker check spacing" above). `WithAnonymousDownload` (RuTracker)
...
`WithPageExport` (Tapochek only so far; issue #186) fetches the topic page the way that plugin's `Check` does — same session, same active domain, same charset decode — and returns `[]registry.PageRegion`: **only the regions its parser reads**, in page order, each as the WHOLE element byte for byte (`forumcommon.TagBlockOuter` keeps the opening tag, whose attributes are evidence). Tapochek exports `title`, `torrent-table` and `opening-post` — exactly the inputs to `Check`, `Download` and `ResolveMetadata`. A region the page did not have comes back with nil HTML and is written into the file as `NOT FOUND on this page`, because a missing torrent table is usually the most useful line in a lost-session report. **It is deliberately not the whole page.** The first version exported everything, and the redactor that had to make it publishable went through five review rounds (three Greptile, two independent codex) with each round finding more — 13 new findings in the last one alone — because "remove every secret from arbitrary HTML and change nothing else" has no finish line. The secrets all live in the page chrome the parser never reads: measured on 2026-09-21, every one of the 12 regions exported live from four tapochek topics carried **zero** secrets before redaction, while each of three full pages of those topics carried four (a `form_token`, two hidden `name="sid"` fields, a `sid=` link). `pageredact` still runs on each region separately as a backstop. Served via `POST /api/v1/topics/{id}/diagnostics/page` (owner-only, per-topic single-flight, a redacted header naming what was included, 422 with nothing on `pageredact.ErrUnsafe`). A new plugin implements it by returning its own parser's regions from its canonicalise-and-fetch pair; a convenient plain GET is NOT acceptable, because a page collected without the session or the decode looks like evidence and is not. `supports_page_export` is surfaced per tracker in `/system/info` so the frontend hides the action rather than offering a control that can only 409.

`WithSearch` (Rutor, RuTracker, LostFilm, Kinozal, Anilibria, Toloka; issue #129)
...
`forumcommon.EncodeWindows1251Query`; note it replaces unencodable runes with
`?`, so a caller that hands it non-UTF-8 text silently searches for
`nm=%3F%3F%3F` and gets a valid-looking zero-result page (the plugin logs the
built URL at Debug for exactly this reason). RuTracker search is **login-gated**:
it needs the handler's Verify-first/Login-on-miss session warm-up, and an
anonymous session gets the login shell → `registry.ErrSearchRequiresCredentials`.
`supports_search` is surfaced per
...
`WithMetadata` (RuTracker, LostFilm, Kinozal, NNM-Club, Anidub, Rutor, Toloka, Tapochek) resolves a real title + poster image
...
`WithAuthorComment` (RuTracker, NNM-Club) fetches the release author's latest
comment from the topic's discussion thread (issue #110). The scheduler calls it
...
`forumcommon/posts.go`; selectors were verified against live topic pages
(2026-07-04). Returning `("", nil)` means "author never commented" — post #1
(the release description) is never treated as a comment.
...
**RuTracker's captcha is adaptive**, unlike LostFilm's: it is imposed on a
client the tracker distrusts (a run of failed logins is enough) and dropped
again after one success. So `Login` posts credentials normally and returns
`registry.ErrCaptchaRequired` only when the response carries `cap_sid`; the
interactive flow is an escalation, not the default path, and
`captchalogin.Engine.Begin` reflects that by harvesting the session silently
when no captcha is demanded. Its captcha is also per-attempt — image on
`static.rutracker.cc` under a content hash, a paired `cap_sid`, and an answer
field named `cap_code_<md5>` — which is why `captchalogin.Config` grew
`ChallengeFrom func(body) (ChallengeSpec, error)`: LostFilm's fixed
`CaptchaURL` + fixed field name cannot express it. `Refresh` re-posts the login
when `ChallengeFrom` is set, because a new picture requires a new `cap_sid`. A
wrong answer re-renders the form (classified `NeedCaptcha`, not
`WrongCaptcha`), and `Complete` treats both as retryable so a typo does not
destroy the pending challenge. `bb_session` is path-scoped to `/forum/`, so
harvesting and rehydrating it must use that path, not the origin root.

...
| **`pageredact`** | strips secrets out of a tracker page before Marauder hands it to a user to attach to a bug report (`POST /topics/{id}/diagnostics/page`). Born from issue #186, where five days went by because the only copy of the page showing the bug belonged to the reporter. Its contract has **two** halves and the second is the one that gets broken: no secret survives, **and everything else is byte-identical** — tags, classes and attributes are what a report is FOR, so a redactor that reformats destroys the evidence. It removes `sid`/`uk`/`passkey`/`token`-class query parameters (matched by NAME, because a session id and a topic id are both hex and only the name says which), hidden `<input>`s whose name says token, `Cookie:` lines some templates echo, and the reporter's own tracker username — whole-word and case-insensitive, never as a substring, or a short account name corrupts unrelated text all over the page. Redaction is best-effort on a page we do not control; the UI says so and asks the user to skim, rather than promising the file is clean |

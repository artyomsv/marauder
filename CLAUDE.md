...
| **`pageredact`** | strips secrets out of a tracker page before Marauder hands it to a user to attach to a bug report (`POST /topics/{id}/diagnostics/page`). Born from issue #186, where five days went by because the only copy of the page showing the bug belonged to the reporter. Its contract has **two** halves and the second is the one that gets broken: no secret survives, **and everything else is byte-identical** — tags, classes and attributes are what a report is FOR, so a redactor that reformats destroys the evidence. It removes `sid`/`uk`/`passkey`/`token`-class query parameters (matched by NAME, because a session id and a topic id are both hex and only the name says which), `<input>` values whose NAME says credential (any `type`, and deliberately not just `hidden` — that attribute is optional and a template may carry a token in a visible field), `Cookie:` lines some templates echo, and the reporter's own tracker username. Both name matches are at **non-letter boundaries, never substrings**: the first version matched `auth`/`key`/`sid` anywhere and blanked `author`, `keywords`, `monkey` and `consideration`, while a short account name matched as a substring corrupts unrelated text all over the page. It reads `<input>` **attributes** rather than expressing order and quoting in one pattern — the single-regex version required `name` before `value` with double quotes on both, so `<input value='x' name='form_token'>` kept the credential (Greptile P1 on PR #193) — and it rewrites only the value, keeping its delimiter, since a changed quote is a markup edit and markup is the evidence. Redaction is best-effort on a page we do not control; the UI says so and asks the user to skim, rather than promising the file is clean |

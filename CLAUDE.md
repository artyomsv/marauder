...
| `plugins/trackers/<name>` | one package per tracker plugin (13 plugins). **Removed 2026-08-03** after probing every advertised domain: `hdclub` (site shut down 2017; `hdclub.org` now 301s to an unrelated file-hosting affiliate site that 404s every TBDev endpoint), `unionpeer` (`.org`/`.com` are parking pages, `.net` NXDOMAIN) and `freetorrents` (`free-torrents.org` has no A record). Do not re-add without live evidence the site serves a topic page |
...
`WithMetadata` (RuTracker, LostFilm, Kinozal, NNM-Club, Anidub) resolves a real title + poster image

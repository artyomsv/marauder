...
export const homeFaq: FaqItem[] = [
  ...
  },
  {
    ...
    answer:
      "Yes. A FlareSolverr container solves the challenge once and returns a clearance cookie plus the browser User-Agent it was issued for; Marauder then replays that pair on its own requests rather than proxying through the browser, which is what lets it submit a login and carry a binary .torrent instead of degrading to a magnet. Start it with the solver overlay, which runs FlareSolverr and points Marauder at it in one step. RuTracker and Kinozal both need this — Kinozal since September 2026, when Cloudflare started challenging its browse and details pages while the site root kept answering normally, which makes the tracker look reachable while every check fails. NNM-Club sits behind Cloudflare too but is not challenged in practice, so it needs no solver at all — and because Cloudflare policy is per-IP, the clearance path stays available if your server does get challenged.",
  },
];
